...
- Funcionalidades implementadas: cadastro de associados, usuários, eleições/enquetes, relatórios diversos, configurações do sindicato, relatórios cartoriais.

### 2. Votação (`frontend/voting`)

- Público: **associados** (votantes).
- Função: PWA completo para votação em eleições/enquetes.
- Login: CPF + data nascimento + matrícula bancária (tripla validação).
- Interface: fluxo otimizado para mobile (login → eleições → votação → comprovante).
- Recursos: offline support, instalável como app nativo, comprovante com QR code.
...
4. **Frontend PWA:** `cd src/frontend/voting`, `npm install` e `npm run dev -- --port 3000`. Login com CPF + Data nascimento + Matrícula bancária.
